...
# 2016.Dec.13th Ver 1.3 Change the check cmd Result of Nuance configuration. Add System_date function.
...
# 2017.May.25th Ver 2.2 Add a printout : echo [`System_date`] Tropo is installed. Need stop it before Nuance deploy.
...
# 2017.Jul.28th Ver 2.9 Add a step to update the Nuance GUI website Administrator password time.
...
if [ "`id -u`" != "0" ]; then
  echo [`System_date`] "You must be a superuser to run this script. Exit now. Pls fix it and rerun this script."
  exit 1
  else
    echo [`System_date`] "Running as superuser."
fi

# Memory must be larger than 1.5G, otherwise NMS can't stop/start
if [ `free -m | grep Mem | awk '{print $2}'` -lt 1500  ]; then
  echo [`System_date`] "The NMS can't stop/start at this host because the memory is less than 1.5G." 
  echo [`System_date`] "Pls increase memory larger than 1.5GB. Exit now. Pls fix it and rerun this script."
  exit 1
fi

# Check whether Nuance Speech Suite is installed
echo [`System_date`] Check the Nuance Speech Suite is installed.
...
if [ `rpm -qa | grep -i nve | wc -l` -lt 2 ];then
  echo [`System_date`] The NVE voice package is not installed. Pls install at least one at first.
  echo "rpm -qa | grep -i nve"
  rpm -qa | grep -i nve
  echo [`System_date`] Pls check the above log and install one NVE voice package. After fix the issue, rerun this script. Exit now.
  exit 1
fi
if [ `rpm -qa | grep -i nre | wc -l` -lt 2 ];then
  echo [`System_date`] The NRE voice package is not installed. Pls install at least one at first.
  echo "rpm -qa | grep -i nre"
  rpm -qa | grep -i nre
  echo [`System_date`] Pls check the above log and install one NRE voice package. After fix the issue, rerun this script. Exit now.
  exit 1
fi

# Check the "hostname -f". Should not be localhost.
if [ "$thishost" = "localhost" ];then
  echo [`System_date`] The \"hostname -f\" is localhost. Pls change it in /etc/hosts. Exit now. Pls fix it and rerun this script.
  exit 1
fi

# Check the Nuance License Manager installed.
if [ -e /etc/init.d/nuance-licmgr ];then
  echo [`System_date`] The Nuance License Manager is installed in this host.
  else
    echo [`System_date`] The Nuance License Manager is NOT installed in this host. Exit now. Pls fix it and rerun this script.
    exit 1
fi
...
if [ "$nuanceLicense" ];then
  echo [`System_date`] The Nuance license file is $nuanceLicense.
  else
    echo [`System_date`] The Nuance License Manager is NOT running. Exit now. Pls fix it and rerun this script.
    exit 1
fi
...
if [ "$hostid_in_licensefile" = "ANY" ];then
  echo [`System_date`] This is a test Nuance license running. The test license will be restricted to 4 ports.
  testflag=yes
  else
    testflag=no
    echo [`System_date`] This is NOT a test Nuance license running. Please input the ports number.
    echo -en "Please input the NVE TTS port number: "
    read tts_port
    echo -en "Please input the NRS ASR port number: "
    read asr_port
fi
echo [`System_date`] The testflag is $testflag.
...
#case $choice in
#  Y|y) testflag=yes
#     echo [`System_date`] This is a test Nuance host. The test license will be restricted to 4 ports.
#  ;;
#  N|n) testflag=no
#
#     echo [`System_date`] This is not a test Nuance host. Input the TTS port number in license.lic
#     echo -en "Please input the NVE TTS port number: "
#     read tts_port
...
#       fi
#
#     echo [`System_date`] This is not a test Nuance host. Input the ASR port number in license.lic
#     echo -en "Please input the NRS ASR port number: "
#     read asr_port
...
#
#  ;;
#  *) echo [`System_date`] Invalid input. Please input Y or N. Your input is $choice. Exit now.
#    exit 1
#  ;;
#esac
...
if [ $serviceStopResult -ne 0 ];then
  echo [`System_date`] $serviceName is not stopped. Manual check by "ps -ef | grep -w $serviceKeyword" and stop it by kill.
fi
}
...
function ReplaceSpecialString(){
if [ ! -f $1 ];then
  echo [`System_date`] $1 does not exist. Pls check Nuance is installed correctly. Exit now. Pls fix it and rerun this script.
  exit 1
fi
#backup
cp $1 $1.`System_date`
...
echo [`System_date`] Replace $3 to $4 in file $1 line $lineNum.
...
if [ "$cmdResultTitle" = "Successful" ];then
  echo [`System_date`] The service-configuration.sh of $1 result is $cmdResultTitle.
  else
    echo [`System_date`] The service-configuration.sh of $1 result is $cmdResultTitle but not Successful.
    echo [`System_date`] The command is not successful executed. Check the printout. Exit now.
    echo [`System_date`] The command printout is below:
    echo [`System_date`] $cmdResult
    exit 3
fi
}
...
echo [`System_date`] Check the Tropo services are stopped.
which tropo_services

case $? in
  0) tropoIsInstalled=yes
     echo [`System_date`] Tropo is installed. Need stop it before Nuance deploy.
     if [ `tropo_services status | grep running | wc -l` -ne 0 ];then
       echo [`System_date`] Tropo is running. Stop it now.
       tropo_services stop
       sleep 30s
       tropoIsStop=yes
     else
       echo [`System_date`] Tropo is not running. Continue deploy Nuance.
       tropoIsStop=yes
     fi
  ;;
  1) tropoIsInstalled=no
     echo [`System_date`] Tropo is not installed. Continue deploy Nuance.
  ;;
  *) echo [`System_date`] Unknown result of \"which tropo_services\". Exit now. Pls fix it and rerun this script.
     exit 1
  ;;
esac

# Stop Nuance service
echo [`System_date`] Stop services: initScriptmserver.sh initScriptmserverdc.sh initScriptmserversa.sh "&" nuance-wd.
...
if [ $? -eq 7 ];then
  echo [`System_date`] The Nuance tomcat is not startup. Manual check. Exit now.
  exit 2
fi
echo [`System_date`] Check the curl result.

# Check the nuance-wd status
echo [`System_date`] Check the nuance-wd status.
service nuance-wd status
if [ $? -ne 0 ];then
  echo [`System_date`] The nuance-wd service is not running normal. Manual check. Exit now.
  exit 3
fi
...
echo [`System_date`] Assign this host as a Nuance server. Expected result is Successful.
...
if [ "$testflag" = "yes"  ];then
  echo [`System_date`] This is a test host, set the NVE TTS port to 4.
cat > /var/local/Nuance/system/config/nvs_update_config.txt <<EOF
tts_license_ports=4
tts_license_ports_overdraft_thresh=4
EOF
  else
    echo [`System_date`] The NVE TTS port is $tts_port, put it in /var/local/Nuance/system/config/nvs_update_config.txt
cat > /var/local/Nuance/system/config/nvs_update_config.txt <<EOF
...
# Add NVE config
echo [`System_date`] Add Nuance Voice Engine config. Expected result is Successful.
cd /usr/local/Nuance/Management_Station/mstation/mserver/webapps/mserver/scripts
...
if [ "$testflag" = "yes"  ];then
  echo [`System_date`] This is a test host, set the NRS license to 4.
cat > /var/local/Nuance/system/config/nrs_update_config.txt <<EOF
swirec_license_ports=4
swiep_license_ports=4
EOF
  else
    echo [`System_date`] The NRS ASR port is $asr_port, put it in /var/local/Nuance/system/config/nrs_update_config.txt
cat > /var/local/Nuance/system/config/nrs_update_config.txt <<EOF
...
# Add NRS config
echo [`System_date`] Add Nuance Recognition Service config. Expected result is Successful.
cd /usr/local/Nuance/Management_Station/mstation/mserver/webapps/mserver/scripts
...
# Add NSS config
echo [`System_date`] Add Nuance Speech Server config. Expected result is Successful.
cd /usr/local/Nuance/Management_Station/mstation/mserver/webapps/mserver/scripts
...
# Update the Nuance GUI  Administrator password if it's a test server
if [ "$testflag" = "yes"  ];then
  echo [`System_date`] This is a test Nuance server and the Nuance GUI Administrator password will be set as the original one.
cat > ./update_Administrator.sql <<EOF
update mserver.t_users set password_changed_time=NOW() where username='Administrator';
EOF
mysql -uroot -p$mysql_password < ./update_Administrator.sql
fi

# Start Tropo services or not
if [[ $tropoIsInstalled = yes && $tropoIsStop = yes && $nuancePort -ne 8080 ]];then
  echo [`System_date`] Start tropo_services now. Please check the result.
  tropo_services start
fi

##############################################################
# Complete, print the result as green
echo -e [`System_date`] "\033[32;49;1m The Nuance Speech Suite is configured and deployed in this host. \033[39;49;0m"

# Inform the user that the Nuance GUI website port
if [ $nuancePort -ne 8080 ];then
  echo -e [`System_date`] NOTES: To avoid 8080 port conflict with Tropo runtime, the Nuance GUI website is set to "\033[31;49;1m \"http://<server ip>:$nuancePost/mserver\" \033[39;49;0m" now. 
  echo -e [`System_date`]And the original Administrator password is
  echo -e "\033[32;49;1m Administrator / changeit \033[39;49;0m"
fi
